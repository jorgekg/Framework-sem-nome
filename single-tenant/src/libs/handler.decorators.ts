...
export const API =
  ...
  (endpoint: string, type: APIEnum, transactional = Transactional.TRANSACTION, auth = Auth.REQUIRED,
    feature: FeaturesEnum = null, permissionLevel = null) =>
...
const processRequest =
  async (instance: CrudHandler, method: Function, req: Request, res: Response, transactional = Transactional.TRANSACTION, auth = Auth.REQUIRED,
    feature: FeaturesEnum, permissionLevel = null) => {
    if (req.body && req.body.toString()) {
      req.body = JSON.parse(req.body);
    }
    const payload = new Payload(req.params.id, req.body, req.query, null, req, res);
    try {
      ...
      if (req.headers['x-api-key']) {
        ...
        Database.initialize();
        // if (feature) {
        //   const permission = await new UserRoleService(payload).getRoleByUser(payload.session.id, feature);
        //   if (false) {
